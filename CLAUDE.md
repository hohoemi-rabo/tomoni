...
- **2本目のゲームは未定**（本採用のものは無い）。ただし汎用化は**実ゲーム4本で通した**——『スーパーマリオブラザーズ』（FC・1985）『ドラゴンクエストIII』（FC・1988）『ゼルダの伝説』（ディスクシステム・1986）『FE 聖戦の系譜』（SFC・1996）を `/knowledge` から登録し、**`src/` を1行も触らずに**実況が成立することを確認した（検証後にすべて削除済み）。
  - **同じシリーズの別作品でも混ざらない**：`knowledge/fe-fc/` が隣にあっても、聖戦（SFC・第4作）に FE1（暗黒竜）の内容は引っ張られなかった。**機種・発売時期を同定のアンカーとして渡す**設計（23）が効いている。
  - **ゲーム固有の語彙も正しく差し替わる**：ロストの概念があるのは4本中 聖戦だけで、`lostLabel` が埋まったのもその1本だけだった。
  - **ゲームの本数に上限は無い**（`listGames` が `knowledge/` を全部読むだけ）。**注入されるのは常にそのプレイスルーの primer 1枚**なので、何本あってもトークンは増えない。上限 3 は**参照URLの数**（`KNOWLEDGE_MAX_URLS`）であって、ゲーム数ではない。
- **AIが「ト書き」を漏らすことがある（要観察・未再現）**：4本検証のうち1本（DQ3）で「この画像は…です。**戦友としては、次のように話しかけます。**「壮大な景色だね…」」と、台本の地の文ごと出力した。読み上げるとそのまま音声になる＝動画では事故。**ただしゲーム画面ではない写真（風景）を投げた検証での話**で、他3本は正常だった。実機（実ゲーム画面）でも出るなら、`prompt.ts` に「地の文・ト書きを書かない」を足す**新チケットを起こす**（先読みで直さない）。
- **実プレイでの値の詰めが残っている**：発話間隔（**45〜90秒**）・返事待ち（90秒）・質問確率（**0.15**）・読み上げ速度（`TTS_SPEAKING_RATE = 1.2`）・発話長（**2〜3文**）。実プレイの手直しで 30〜60秒→45〜90秒・0.3→0.15・声を1.2倍・3〜4文→2〜3文に振った（`427592f`）。返事待ちは**そのまま動画の無音**になるので、実際に撮ってみて詰める。
...
- **自動実況ループ（04・15・18・19・22）**: `src/lib/video/frame.ts`（`captureFrame`／`signatureDiff`）・`src/lib/turn.ts`（`pickTurnKind`／`looksLikeQuestion`・**純関数**。乱数は引数で注入するのでブラウザ抜きで検証できる）・`src/hooks/useAutoNarration.ts`（間隔ループ・多重送信抑止・手動トリガー・`recentLines` 保持。送信は `onSend: (p: SendPayload) => Promise<string | void>` で注入。**確定した発言を返す**＝質問で終わったかの判定に使う。`SendPayload` は `{ imageBase64, recentLines, userMessage?, turnKind }`）。tick の関門は**4つ**で、順に **①生成中(`busyRef`) → ②`canSpeak()`（読み上げ中でない・18／**プレイヤーが STT で話している最中でない**・`427592f`） → ④返事待ちでない（22） → ③時間（前回発話から `gapRef` の乱数間隔が経過・19）**。抜けたら必ず送る。**何を喋るかは `pickTurnKind` が決める**（話しかけ優先／確率 `QUESTION_TURN_PROBABILITY`（実プレイで 0.3→**0.15**）で質問・ただし直前の発言が `?` で終わっていたら抽選しない／外れたら差分で実況・雑談。差分はここにしか効かない。テンポの門番にすると取り込みノイズとカーソル点滅で喋り出す＝実測）。乱数間隔は `send()` 内の `rollGap()` で毎回引き直す（唯一の送信集約点なので手動・STT の直後もループが被せてこない）。`canSpeak` は SessionClient が `!tts.speaking && tts.queueLength === 0 && !sttListeningRef.current` を渡す（**読み上げ中に加え、プレイヤーが返事を話している間も黙る**）。**この門番は返事待ちの計測より手前にあるので、プレイヤーが話している間はタイムアウトのカウントも進まない**（話し終えて確定テキストが出れば `triggerNow` が返事待ちを解除する）。**`triggerNow`（手動・STT）には②③④を掛けず、返事待ちも解除する**（自分で押したのに黙るのは故障に見える）。**返事待ちの計測は「質問の読み上げが終わってから」**（読み上げ中から計ると長い質問ほど待ちが短くなる）。タイムアウトで `turnKind: 'giveup'` を1回だけ送り通常ループへ戻る＝**黙り込ませない**。
...
- **プレイヤーの返事に被せて喋る（`427592f` で実測）。** `canSpeak` が TTS だけを見て `stt.listening` を見ていなかったため、返事待ちのタイムアウト（giveup）や通常の実況が、プレイヤーが答えている途中で喋り出した（＝返事を途中で切る）。`canSpeak` を `stt.listening` でも閉じる。この門番は返事待ちの計測より手前なので、話している間はタイムアウトのカウントも止まる（話し終えて確定テキストが出れば `triggerNow` が返事待ちを解除する）。**STT は Web Speech・マイク依存で無人駆動できない**ので、この修正は開発者のマイク確認に委ねる。
