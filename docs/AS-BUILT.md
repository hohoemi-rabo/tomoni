# AS-BUILT — 実装仕様書（2026-07-14 時点・チケット20まで）
...
- 対象範囲：**チケット 01〜20＝実装完了ぶん**。22・21・23（この順で実装予定）は未着手なので「未実装」として §11 に記す。
...
├── game.json              呼び方だけ（title / version / progressLabel / progressPlaceholder / lostLabel）
...
- **この2本はまだ FE 固定**（抽出スキーマも保存先も `fe-fc`）。ゲームごとに差せるようにするのはチケット21。
...
- **チケット22（会話形式）**：`turnKind`（`'narrate' | 'chat' | 'question' | 'giveup'`）・質問ターンの抽選・返事待ち（関門4）・タイムアウトでの切り上げ。**現状は `isIdle`（真偽値）とターン指示2定数のまま**で、AIから質問はしない。
- **チケット21（`/knowledge` のゲームごと抽出設定）**：`/api/knowledge/extract` と `/save` は**まだ FE 固定**（抽出スキーマが章＋自軍/敵の SRPG 形、保存先が `fe-fc` 固定）。
- **チケット23（ゲーム登録）**：`/knowledge` からの primer 下書き生成。現状 primer は手書き。
