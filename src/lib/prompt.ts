...
export function buildSystemPrompt({
  ...
  sections.push(
    [
      ...
      "- 攻略はしない。最適な一手・正解ルート・加入条件を満たす操作手順（誰で話しかける等）を指示しない。聞かれても答えを渡さず、一緒に悩む側に回る。",
      "- 先の展開や人物の運命は、戦友として自分から語ってよい（例:「あの剣士はナバール。実は仲間になるんだ」）。語るのは事実まで。仲間にする手順は言わない。",
      ...
      "- 固有名は慎重に。画面に名前・ステータス等の文字が出ていない限り、いま映っているユニットが誰かを断定しない。分からなければ「自軍のユニット」「敵」と呼ぶ。文字が出ている画面でのみ、現在章のキャスト表と照合して特定する。これは画面内のユニットの特定についての制約であり、物語や人物を知識として名前を挙げて語ることは妨げない。",
